...
if __name__ == "__main__":
    ...
    if model_dirs:
        with st.form(key="my_form"):
            ...
            best = col2.number_input(
                "Number of models", key="num_models", step=1, min_value=1, value=5
            )
            # Every form must have a submit button.
            submitted = st.form_submit_button("Get Results")
            if submitted:
                model_dfs, parameters = calculate_model_metrics(
                    model_dirs, experiment, best
                )

                # concatenate the dataframes
                df_concat_avg_w_var = pd.concat(
                    [model_dfs[k] for k in model_dfs.keys() if experiment in k]
                )

                # calculate the mean of each evaluation metric
                df_avg_w_var = df_concat_avg_w_var.groupby(
                    ["model", "variables"]
                ).mean()
                model_var_counts = (
                ...
                df_avg_w_var = (
                    df_avg_w_var.reset_index()
                )  # reset index so that 'model' and 'variables' become regular columns
                df_avg_w_var = pd.merge(
                    df_avg_w_var, model_var_counts, on=["model", "variables"]
                )
